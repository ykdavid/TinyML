...
/* Includes ---------------------------------------------------------------- */
#include <TinyGesture_IMU_Gesture_Control_inferencing.h>
...
// Confidence threshold: predictions below this value will be ignored.
// For live demo, 0.70-0.80 is a reasonable range.
const float CONFIDENCE_THRESHOLD = 0.70;

// Set this to true if you only want the stable demo commands:
// idle = off, circle = purple, move_down = yellow.
// Other labels will be printed but will not change the LED.
const bool STABLE_DEMO_MODE = false;

// Wait 3 seconds after the user confirms before starting each inference.
// Note: the real model input window is still defined by the Edge Impulse model.
const unsigned long PRE_INFERENCE_DELAY_MS = 3000;

// Function declarations
void setBuiltInRGB(bool red, bool green, bool blue);
void applyGestureCommand(const char *label, float confidence);
void waitForNextInferenceCommand();
...
void setup()
{
    ...
    // Nano 33 BLE Sense onboard RGB LED is active-low:
    // LOW = on, HIGH = off
    ...
    ei_printf("After each prediction, send 'n' or press Enter in Serial Monitor to run the next inference.\r\n");
}
...
void waitForNextInferenceCommand() {
    ei_printf("\r\nReady for next inference. Send 'n' or press Enter to start.\r\n");

    while (true) {
        if (Serial.available() > 0) {
            char c = Serial.read();

            // Pressing Enter in Serial Monitor usually sends '\r' and/or '\n'.
            if (c == 'n' || c == 'N' || c == '\r' || c == '\n') {
                // Clear any remaining characters from the input buffer.
                // A short delay helps consume NL/CR characters that arrive just after 'n'.
                delay(30);
            ...
            ei_printf("Unknown command. Send 'n' or press Enter to start the next inference.\r\n");
        }
...
void setBuiltInRGB(bool red, bool green, bool blue) {
    ...
    // LOW = on, HIGH = off.
    // First turn all channels off briefly. This prevents the previous color
    // from visually persisting and makes each new command easier to see.
    digitalWrite(LEDR, HIGH);
    digitalWrite(LEDG, HIGH);
    digitalWrite(LEDB, HIGH);
    delay(20);
...
void applyGestureCommand(const char *label, float confidence) {
    if (confidence < CONFIDENCE_THRESHOLD) {
        ei_printf("LED command skipped: confidence %.5f is below threshold %.2f\n", confidence, CONFIDENCE_THRESHOLD);
        return;
    }

    if (STABLE_DEMO_MODE) {
        if (strcmp(label, "idle") != 0 && strcmp(label, "circle") != 0 && strcmp(label, "move_down") != 0) {
            ei_printf("LED command ignored in stable demo mode: %s\n", label);
            return;
        }
    }

    if (strcmp(label, "idle") == 0) {
        setBuiltInRGB(false, false, false);   // off
    }
    else if (strcmp(label, "swipe_left") == 0) {
        setBuiltInRGB(true, false, false);    // red
    }
    else if (strcmp(label, "swipe_right") == 0) {
        setBuiltInRGB(false, true, false);    // green
    }
    else if (strcmp(label, "move_up") == 0) {
        setBuiltInRGB(false, false, true);    // blue
    }
    else if (strcmp(label, "move_down") == 0) {
        setBuiltInRGB(true, true, false);     // yellow
    }
    else if (strcmp(label, "circle") == 0) {
        setBuiltInRGB(true, false, true);     // purple
    }
    else {
        ei_printf("LED command skipped: unknown label %s\n", label);
        return;
    }

    ei_printf("LED command updated immediately: %s, confidence = %.5f\n", label, confidence);
}

void loop()
{
    waitForNextInferenceCommand();

    // Clear the previous LED color at the beginning of each new trial.
    // The next color will only appear after the new prediction is available.
    setBuiltInRGB(false, false, false);

    ei_printf("\nStarting inferencing in 3 seconds...\r\n");
    delay(PRE_INFERENCE_DELAY_MS);
    ...
    ei_printf("Best prediction: %s, confidence = %.5f\n", bestLabel, bestConfidence);
    applyGestureCommand(bestLabel, bestConfidence);
    ei_printf("Inference finished.\r\n");
